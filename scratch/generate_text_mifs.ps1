...
function Generate-MIF {
    ...
    # Enable anti-aliasing for smooth text (but we'll threshold it to 1-bit later)
    ...
    for ($y = 0; $y -lt $height; $y++) {
        for ($x = 0; $x -lt $width; $x++) {
            ...
            # Black pixel is text (1), white pixel is background (0)
            if ($pixel.R -lt 128) {
...
Generate-MIF -text "PRESS ENTER TO START" -filename "c:\Quartus_Projects\shared_project\MIF\start_text.mif" -width 512 -height 64 -fontSize 36
Generate-MIF -text "GAME OVER!" -filename "c:\Quartus_Projects\shared_project\MIF\game_over_text.mif" -width 512 -height 64 -fontSize 48
