    ...
    always_ff @(posedge clk or negedge resetN) begin
        if (!resetN) begin
            ...
            if (reset_score_pulse) begin
                ...
                case (pulled_id)
                    3'd1: score <= score + (10 * pulled_weight * pulled_weight); // Cop
                    3'd2: score <= score + (50 * pulled_weight * pulled_weight); // Robber Stand
                    3'd3: score <= score + (50 * pulled_weight * pulled_weight); // Robber Run
                    3'd4: score <= score + 16'd500;  // Maryjane
                    3'd5: score <= score + 16'd1000; // Riddler
                    3'd6: score <= score + 16'd1000; // Goblin
                endcase
            end
        end
    end
